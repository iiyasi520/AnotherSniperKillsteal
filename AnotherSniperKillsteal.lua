...
AnotherAutoAssassinate.optionKey = Menu.AddKeyOption({"Hero Specific","Sniper"},"Another Auto Assassinate Key",Enum.ButtonCode.KEY_D)
...
function AnotherAutoAssassinate.OnUpdate()
	...
	if Menu.IsKeyDown(AnotherAutoAssassinate.optionKey) then
...
function AnotherAutoAssassinate.FindTarget(me, ability)
	...
	--Log.Write(NPC.GetAttackRange(me))
	local AbsDMG = UltimateDamage + UltimateDamage * (Hero.GetIntellectTotal(me) / 16 / 100)
	local entities = Heroes.GetAll()
	for index, ent in pairs(entities) do
		local enemyhero = Heroes.Get(index)
		if not Entity.IsSameTeam(me, enemyhero) and not NPC.IsLinkensProtected(enemyhero) and not NPC.IsIllusion(enemyhero) and NPC.IsEntityInRange(me, enemyhero, Ability.GetCastRange(ability) + NPC.GetCastRangeBonus(me)) and not NPC.IsEntityInRange(me, enemyhero, NPC.GetAttackRange(me) + DragonLance) then
			local totaldmg = (1 - NPC.GetMagicalArmorValue(enemyhero)) * AbsDMG
